import React, { useEffect, useState, useRef } from 'react';
import { ArrowLeft, Search } from "lucide-react";
import { useNavigate } from 'react-router-dom';
import { getUserProfile, updateProfile } from '../services/apiService';

const ProfilePage = () => {
    const [user, setUser] = useState(null);
    const [Profile, setProfile] = useState(null);
    const navigate = useNavigate();
    const fileInputRef = useRef(null);

    const fetchUserData = async () => {
        try {
            const res = await getUserProfile();
            if (res?.user) {
                setUser(res.user);
            }
        } catch (error) {
            console.error("Error fetching profile:", error);
        }
    };

    useEffect(() => {
        fetchUserData();
    }, []);



    const uploadFile = async (file) => {
        const formData = new FormData();
        formData.append('profilePic', file);

        try {
            const res = await updateProfile(formData); // Must return JSON, not Response

            if (res.error) {
                throw new Error(res.error);
            }

            console.log("Profile picture uploaded successfully:", res);
            setUser(prev => ({ ...prev, profilePic: res.profilePic }));

        } catch (error) {
            console.error("Error uploading profile picture:", error);
        }
    };


    const handleFileChange = (e) => {
        const selected = e.target.files[0];
        if (selected) {
            uploadFile(selected);
            setProfile(selected);

        }
    };

    console.log("Selected file:", Profile);

    if (!user) return <div className="text-white text-center mt-20">Loading profile...</div>;


    return (
        <div className="w-full text-white">
            {/* Header */}
            <div className="flex justify-between items-center p-5 border-b border-gray-800">
                <div className="flex items-center">
                    <ArrowLeft
                        onClick={() => navigate(-1)}
                        className='cursor-pointer text-white h-6 w-6'
                    />
                    <div className="ml-3">
                        <h3 className='text-xl font-bold'>{user.userName}</h3>
                        <h6 className='text-gray-400 text-sm'>{user.tweets?.length || 0} posts</h6>
                    </div>
                </div>
                <Search className='cursor-pointer text-white h-6 w-6' />
            </div>

            {/* Cover Photo (optional) */}
            <div className="h-40 bg-gray-700 w-full"></div>

            {/* Profile Picture & Edit */}
            <div className="relative px-5 cursor-pointer" onClick={() => fileInputRef.current.click()}>
                <img
                    src={user.profilePic || "https://abs.twimg.com/sticky/default_profile_images/default_profile_400x400.png"}
                    alt="Profile"
                    className="w-24 h-24 rounded-full border-4 border-black -mt-12 object-cover"
                />
                <input
                    ref={fileInputRef}
                    type="file"
                    accept="image/*,video/*"
                    onChange={handleFileChange}
                    className="hidden"
                />
            </div>

            {/* Info Section */}
            <div className="px-5 mt-2">
                <h2 className="text-xl font-bold">{user.userName}</h2>
                <p className="text-gray-500">@{user.email?.split("@")[0]}</p>
                <p className="text-sm mt-2">{user.bio || "No bio available."}</p>
                <p className="text-sm text-gray-400 mt-1">Joined {new Date(user.createdAt).toLocaleDateString()}</p>

                <div className="flex gap-4 mt-2 text-sm">
                    <span><strong>{user.following?.length || 0}</strong> Following</span>
                    <span><strong>{user.followers?.length || 0}</strong> Followers</span>
                </div>
            </div>

            {/* Tweets section */}
            <div className="mt-4 px-5">
                <h3 className="text-lg font-semibold border-b border-gray-700 pb-2 mb-2">Tweets</h3>
                {user.tweets?.length > 0 ? (
                    user.tweets.map((tweet, index) => (
                        <div key={index} className="border-b border-gray-800 pb-4 mb-4">
                            <p>{tweet.content}</p>
                            <span className="text-xs text-gray-500">{new Date(tweet.createdAt).toLocaleString()}</span>
                        </div>
                    ))
                ) : (
                    <p className="text-gray-400">No tweets yet.</p>
                )}
            </div>
        </div>
    );
};

export default ProfilePage;
